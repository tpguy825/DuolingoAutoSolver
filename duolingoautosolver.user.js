	...
	if (selNext.length === 1) {
			...
			if (window.sol.correctIndices) {
				...
				nativeInputValueSetter.call(
					...
					window.sol.correctSolutions
						? window.sol.correctSolutions[0].replace(article + " ", "")
						: window.sol.displayTokens
							? window.sol.displayTokens.find((t) => t.isBlank).text
							: window.sol.prompt,
				);
		...
		if (window.sol.type == "tapComplete") {
			...
			function click(answer) {
				console.log(
					...
					window.sol.choices[answer].text,
				);
		...
		if (window.sol.type == "translate") {
			...
			if (bank) {
				...
				function click(answer) {
					const item = options.find((op) => op.innerText == correct[answer]);
		...
		if (document.querySelectorAll('[data-test$="challenge-tap-token"]').length > 0) {
			...
			if (window.sol.pairs) {
				...
				try {
					...
					window.sol.correctIndices.forEach((index) => {
						let choices = (i) =>
							window.sol.correctTokens ? window.sol.correctTokens[i] : window.sol.choices[i].text;
		...
		if (window.sol.type == "partialReverseTranslate") {
			const totype = window.sol.displayTokens
				.filter((t) => t.isBlank)
				.map((t) => t.text)
				.join("");
